<div class="col-md-3 col-sm-3 col-xs-12 left_sidebar">
	<div class="sidebar_search">
		<form class="search_form" action="{{route('web.search')}}" method="post">
			...
			<button type="submit" class="search_submit_btn"><i class="cangle-right fa fa-search"></i></button>
		</form>
	</div>
	<div class="sidebar_category">
		<h3 class="title-color">Editors</h3>
		<ul>
			@foreach($sidebar['board_categories'] as $bcat)
			<li><a href="{{route('web.all-editorial-category', ['categorySlug'=>$bcat->slug])}}"><i class="cangle-right fa fa-angle-right"></i>{{$bcat->name}}</a></li>
			@endforeach
		</ul>
	</div>
	<div class="sidebar_category">
	...
	<div class="sidebar_category">
		...
		<ul>
			...
			<li><a href="{{$news->url}}"><i class="cangle-right fa fa-angle-right"></i>{{$news->name}}</a></li>
			@endforeach
		</ul>
	</div>
	@endif
